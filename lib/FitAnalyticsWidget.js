...
class FitAnalyticsWidget extends Component {
  ...
  /* PUBLIC INTERFACE */
  reconfigure(options) {
    this.sendAction('reconfigure', [ options ])
  }

  getRecommendation(options) {
    this.sendAction('getRecommendation', [ options ])
  }

  open(options) {
    this.sendAction('open', [ options ])
  }

  close(options) {
    this.sendAction('close', [ options ])
  }
  ...
  componentDidUpdate(prevProps) {
    ...
    if (shouldReconfigure)
      this.reconfigure(this.buildWidgetOptions(changedProps))
  }
  ...
  decodeMessage (str) {
    var out = null
    try {
      out = JSON.parse(decodeURIComponent(escape(atob(str))))
    }
    catch (err) {
      console.error(err)
    }
  ...
  onWebViewMessage(data) {
    ...
    if (msg != null && msg.action != null) {
      ...
      case "__ready":
        ...
        this.sendAction("init", [ this.buildWidgetOptions(this.props) ])
        break
      case "__init":
      ...
      case "load":
        if (args != 0 && args.length > 0) {
          ...
          if (details.sid != null) {
            try {
              AsyncStorage.setItem("fita:sid", details.sid).then((res) => {
                // noop
              })
            } catch (err) {
  ...
  render() {
    return <WebView
      ref={this.webview}
      source={{ uri: 'https://widget.fitanalytics.com/widget/app-embed.html' }}
      style={{
        position: 'relative',
        width: '100%',
        height: '100%',
